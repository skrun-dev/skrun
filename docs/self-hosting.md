...
The `Content-Type` header is not decoration: a change-making request that carries the session cookie, sends a form-style body and shows no browser signal is refused with `403`, because that is indistinguishable from a request another site forged. Naming a non-simple type is enough to pass; so is using an `sk_live_` key, which is what this command exists to create.

On a deployment that configures a session cookie domain **and** runs in production, the cookie's name is `__Secure-skrun_session` rather than `skrun_session` — see `SKRUN_SESSION_COOKIE_DOMAIN` below. Everywhere else, including every deployment that configures nothing, it stays `skrun_session`.
...
| `SKRUN_PUSH_MAX_BODY_MB` | `50` | Cap on the **compressed** push request body, applied before the body is buffered — over it, `POST /api/agents/:ns/:name/push` returns `413 BUNDLE_TOO_LARGE`. Distinct from `BUNDLE_MAX_DECOMPRESSED_MB` above, which bounds what the archive expands to once accepted; both apply. Must be a positive number — the server refuses to boot on anything else rather than fall back silently. |
...
| `SKRUN_PUBLIC_URL` | — (derived from `X-Forwarded-Proto` / `Host`) | The canonical public origin of this deployment, e.g. `https://api.example.com` (a port is allowed; a path, query or fragment is not). When set it is the **first** source for every URL the server hands out — the GitHub OAuth `redirect_uri`, the device-login verification URL, and the `servers` entry of the published schema — so those stop depending on a `Host` header the caller chooses. The server refuses to boot on a malformed value rather than guess. **Required** as soon as `SKRUN_SESSION_COOKIE_DOMAIN` is set. |
| `SKRUN_SESSION_COOKIE_DOMAIN` | — (host-only cookie) | Registrable domain to scope the browser session cookie to, e.g. `example.com`, so a dashboard on one host and the API on another share one sign-in. Leave unset — the default — and the cookie stays host-only exactly as before. When set, the value must be a registrable domain of which the canonical host is the domain itself or a subdomain (`api.example.com` under `example.com`), never a public suffix (`co.uk`, `fly.dev`) and never a bare IP; anything else is a boot failure with a message naming what it got and what it expected. **Two consequences to plan for.** In production the cookie is renamed to `__Secure-skrun_session`, so everyone signs in once more, once; and a finished browser login is redirected to the root of that domain — the scheme of `SKRUN_PUBLIC_URL`, without the API's port — instead of `/dashboard`. Changing the domain later leaves every signed-in browser with a cookie on the old scope that this server can no longer erase: users clear it by hand, or sign out before the change. |
| `SKRUN_API_KEY_TTL_DAYS` | `90` | Lifetime of a key minted by `skrun login`, in days from creation. `0` = those keys never expire. Applies **only** to the CLI login flow: a key created through `POST /api/keys` expires only if the caller passes `expires_at`. Never retroactive — keys that already exist keep no expiry. A malformed or negative value falls back to 90 rather than disabling expiry. |
...
| `FILES_RETENTION_S` | `3600` | How long agent-produced files are kept (seconds). Expired files are deleted by a sweep that runs every five minutes, and directories left by a previous process are removed at boot — the window is a deletion bound, not just an availability |
...
| `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN` | — | When both set, the `push` / `run` rate limiter uses Upstash Redis so the per-IP limit is **shared across instances** (multi-instance deployments). Unset → in-memory limiter (correct for a single instance). Each rate-limited route keeps its own counter in that store (`push`, `run`, the two device-login mounts), so exhausting one never spends another. On a Redis outage the limiter falls back to in-memory rather than failing open or closed. |
| `SKRUN_TRUST_PROXY` | — (off) | Set (`1`/`true`/`on`) when the server sits behind a reverse proxy / load balancer, so the rate-limit client address is read from a forwarded header instead of the socket. **Both shipped compositions already carry it** on the `api` service — `docker-compose.yml` sets it, and the `docker-compose.minimal.yml` overlay inherits it (the overlay keeps Caddy in front). In both, the api is reachable only through Caddy, and the shipped `Caddyfile.example` **overwrites** `X-Forwarded-For` with the address it saw — that overwrite is what makes the header trustworthy. Leave unset when the server is directly exposed: the header is then ignored, because a caller would otherwise choose their own rate-limit key. If your own gateway **appends** to `X-Forwarded-For` instead of overwriting it, do not stop here — set `SKRUN_TRUST_PROXY_HEADER` too. |
| `SKRUN_TRUST_PROXY_HEADER` | — (use `X-Forwarded-For`) | Name of the header your gateway writes the client address into itself — take the name from that gateway's own documentation (`Fly-Client-Ip` on Fly.io, `CF-Connecting-IP` behind Cloudflare, whatever your load balancer documents). Set it when your gateway **appends** to `X-Forwarded-For` rather than overwriting it: the first hop of that header is then the value the caller sent, so keying the limit on it lets one caller mint a fresh counter per request by varying what it sends. Case-insensitive. Read only when `SKRUN_TRUST_PROXY` is on. When the variable is set but the header is missing from a request, that request did not arrive through the named gateway: the limiter falls back to the socket address — degraded (one counter for every caller on that path) but never caller-controlled. It never falls back to `X-Forwarded-For`, which would hand the key back to the caller. |
...
> **Before you set `SKRUN_SESSION_COOKIE_DOMAIN`.** A cookie scoped to a parent domain is only as safe as the whole zone: **every** host under that domain can set a cookie the server will receive, so sharing a session across `example.com` is sound only if you control every host in that zone and delegate none of it — one compromised or third-party sibling host is enough to plant a session cookie for the others. It is also a **production** configuration: outside production the cookie travels without the `Secure` flag, no name prefix applies, and the server does not detect a planted duplicate. Leave the variable unset if the dashboard and the API share a host; you lose nothing.
